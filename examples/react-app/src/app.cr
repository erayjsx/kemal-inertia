...
def vite_tags(entry : String)
  # Check if we are in production or if manifest exists and we want to use it
  # For this example, we'll prefer the manifest if we are in production mode
  # OR if we are in development but the manifest exists and we can't connect to dev server?
  # Simpler: Use manifest if KEMAL_ENV is production.
...
# In production, you would typically serve 'dist' folder or use Nginx
serve_static({"gzip" => true, "dir_listing" => false})

get "/" do |env|
  Kemal::Inertia.render(env, "Home", name: "Inertia + React")
end
